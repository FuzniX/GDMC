...
class House:
    ...
    def build_shape(self) -> None:
        ...
        """
        # Build walls
        placeCuboidHollow(self.editor, (0, 0, 0), (self.width, self.height, self.depth), self.wallBlock)

        # Place blocks below to fit terrain
        placeCuboid(self.editor, (0, 0, 0), (self.width, -5, self.depth), self.floorPalette)
    ...
    def build_door(self) -> None:
        ...
        placeCuboid(self.editor, (doorPosition - 1, 1, -1), (doorPosition + 1, 3, -1), Block("air"))
    ...
    def build_roof(self) -> None:
        ...
        for dx in range(1, self.halfWidth + 2):
            ...
            placeCuboid(self.editor, (self.halfWidth - dx, yy, 0), (self.halfWidth + dx, yy, self.depth),
                        self.wallBlock)

            # Build row of stairs blocks
            leftBlock = Block(roofStairs.id, {"facing": "east"})
            rightBlock = Block(roofStairs.id, {"facing": "west"})
            placeCuboid(self.editor, (self.halfWidth - dx, yy, -1), (self.halfWidth - dx, yy, self.depth + 1),
                        leftBlock)
            placeCuboid(self.editor, (self.halfWidth + dx, yy, -1), (self.halfWidth + dx, yy, self.depth + 1),
                        rightBlock)
        ...
        placeCuboid(self.editor, (self.halfWidth, yy, -1), (self.halfWidth, yy, self.depth + 1), roofBlock)

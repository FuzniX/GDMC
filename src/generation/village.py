...
class Village:
    ...
    def build(self) -> None:
        ...
        self.build_pirate_decorations(min_distance=12)
    ...
    def build_pirate_decorations(self, min_distance: int = 6) -> None:
        """
        Spawn scattered totems across the pirate zone.
        Ensures they don't overlap with buildings and sit on top of the ground.
        """
        pirate_coords = np.argwhere(self.houseMap == PIRATE_BLOCK)

        if len(pirate_coords) == 0:
            return

        # Shuffle to randomize placement order
        np.random.shuffle(pirate_coords)

        placed_positions = [] 

        for coord in pirate_coords:
            x, z = coord[0], coord[1]

            # Safety check: skip if a house exists here
            if self.houseMap[x, z] == HOUSE_BLOCK:
                continue

            # Check distance against existing props
            too_close = False
            for px, pz in placed_positions:
                # Euclidean distance check
                if math.sqrt((x - px)**2 + (z - pz)**2) < min_distance:
                    too_close = True
                    break
            ...
            ground_y = int(self.heightmaps[x, z]) + 1

            # Place fence post
            self.editor.placeBlock((x, ground_y, z), Block("spruce_fence"))

            # Place skull on top
            skull_rotation = str(randint(0, 15))
